...
global $pagenow, $mpifw_mwb_mpifw_obj;
...
$mpifw_onboarding_form_deactivate = 
// desc - Deactivation template form fields.
apply_filters('mwb_mpifw_deactivation_form_fields', array());

?>
<?php if ( ! empty( $mpifw_onboarding_form_deactivate ) ) : ?>
	<div id="<?php echo esc_attr( $mwb_plugin_onboarding_popup_id ); ?>"
			...
			<div class="mwb-mpifw-on-boarding-wrapper mdc-dialog__surface" role="alertdialog" aria-modal="true" aria-labelledby="my-dialog-title" aria-describedby="my-dialog-content">
				<div class="mdc-dialog__content">
					<div class="mwb-mpifw-on-boarding-close-btn">
						<a href="#">
							<span class="mpifw-close-form material-icons mwb-mpifw-close-icon mdc-dialog__button" data-mdc-dialog-action="close">clear</span>
						</a>
					</div>

					<h3 class="mwb-mpifw-on-boarding-heading mdc-dialog__title"></h3>
					<p class="mwb-mpifw-on-boarding-desc"><?php esc_html_e( 'May we have a little info about why you are deactivating?', 'mwb-paypal-integration-for-woocommerce' ); ?></p>
					<form action="#" method="post" class="mwb-mpifw-on-boarding-form">
						<?php 
						$mpifw_onboarding_deactive_html = $mpifw_mwb_mpifw_obj->mwb_mpifw_plug_generate_html( $mpifw_onboarding_form_deactivate );
						echo esc_html( $mpifw_onboarding_deactive_html );
						?>
						<div class="mwb-mpifw-on-boarding-form-btn__wrapper mdc-dialog__actions">
							...
							<div class="mwb-mpifw-on-boarding-form-no_thanks">
								<a href="#" id="<?php echo esc_attr( $mwb_plugin_deactivation_id ); ?>"
								class="
								<?php
								echo 
								// desc - filter for trial.
								wp_kses_post( apply_filters('mwb_stand_no_thank_classes', 'mwb-paypal-integration-for-woocommerce-no_thanks' ) );
								?>
								mdc-button"><?php esc_html_e( 'Skip and Deactivate Now', 'mwb-paypal-integration-for-woocommerce' ); ?></a>
							</div>
						</div>
					</form>
				</div>
